...
def password_reset_request(request):
    ...
    if request.method == 'POST':
        ...
        try:
            ...
            # Log request payload (safe to log email; do not log secrets)
            logger.info('Sending Firebase password reset request for email: %s', email)
            response = requests.post(url, json=payload)
            logger.info('Firebase password reset response: status=%s body=%s', response.status_code, response.text)
            ...
            logger.exception('Password reset request error for email %s: %s', email, e)
            return render(request, 'accounts/password_reset_request.html')
...
def password_reset_confirm(request):
    ...
    if request.method == 'POST':
        ...
        try:
            ...
            logger.info('Sending Firebase password reset confirm for reset_code (masked): %s', reset_code[:6] if reset_code else '')
            response = requests.post(url, json=payload)
            logger.info('Firebase password reset confirm response: status=%s body=%s', response.status_code, response.text)
            ...
            messages.error(request, f'An error occurred: {str(e)}')
            logger.exception('Password reset confirm error: %s', e)
            return render(request, 'accounts/password_reset_confirm.html')
